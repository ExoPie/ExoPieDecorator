...
TFile* f_TF = new TFile("TF.root","READ");
...
void createRegion(RooRealVar met, TH1F* h_sr_bkg , TH1F* h_cr_bkg,
  ...
  */
  ...
  for (int isys=0; isys < (int) nuisIndex.size(); isys++){
    ...
    */
    for (int i =0; i<6; i++)  std::cout<<" add_logN_systematic = "<<add_logN_systematic[i]<<std::endl;
    ...
    rfv_bin6 += "*"+add_logN_systematic[5];
    //rfv_bin7 += "*"+add_logN_systematic[6];
    //rfv_bin8 += "*"+add_logN_systematic[7];
    ...
    std::cout<<" bin 6 stats unc "<< rfv_bin6<<" after including "<<nuisanceName[nuisIndex[isys]]<<std::endl;
    //std::cout<<" bin 7 stats unc "<< rfv_bin7<<" after including "<<nuisanceName[nuisIndex[isys]]<<std::endl;
    //std::cout<<" bin 8 stats unc "<< rfv_bin8<<" after including "<<nuisanceName[nuisIndex[isys]]<<std::endl;
    ...
    // extend to 8 bins
    ral_bin5.add(*rrv_syst);
    ral_bin6.add(*rrv_syst);
    //ral_bin7.add(*rrv_syst);
    //ral_bin8.add(*rrv_syst);
  ...
  std::cout<<" bin 5 total unc is "<<rfv_bin5<<std::endl;
  std::cout<<" bin 6 total unc is "<<rfv_bin6<<std::endl;
  //std::cout<<" bin 7 total unc is "<<rfv_bin7<<std::endl;
  //std::cout<<" bin 8 total unc is "<<rfv_bin8<<std::endl;
...
void PrepareWS_withnuisanceInvertTF_noW_6Bins(TString model_="monoHbb",TString analysiscategory_="merged", TString mode__ = "RECREATE", TString inputdir=".", TString inputfile="AllMETHistos.root", TString year="2016", int nbins=4,TString version     = "_V0"){
