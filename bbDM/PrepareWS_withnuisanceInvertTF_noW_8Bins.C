...
void createRegion(RooRealVar met, TH1F* h_sr_bkg , TH1F* h_cr_bkg,
  ...
  */
  ...
  for (int isys=0; isys < (int) nuisIndex.size(); isys++){
    ...
    */
    for (int i =0; i<8; i++)  std::cout<<" add_logN_systematic = "<<add_logN_systematic[i]<<std::endl;
...
void PrepareWS_withnuisanceInvertTF_noW_8Bins(TString model_="monoHbb",TString analysiscategory_="merged", TString mode__ = "RECREATE", TString inputdir=".", TString inputfile="AllMETHistos.root", TString year="2016", int nbins=4,TString version     = "_V0"){
